...
@code{.c}
int8_t data_access_ptr(tenuFlashDataFnCtl enuCtl, void *pvStr)
{
    // Return value.
    int8_t           s8Ret = -1;
    ...
    static uint32_t       u32BytesRemaining = 0;
...
@endcode
*/
typedef int8_t (*tpfDataAccessFn)(tenuFlashDataFnCtl enuCtl, void *pvStr);
...
@return     Zero indicates success.
            Negative values indicate failure.
*/
int8_t m2m_flash_rootcert_add(uint16_t u16Id, tpfDataAccessFn pfFn, uint32_t u32Size);
...
@code{.c}
{
    ...
    if (status == 0)
    {
        ...
        if (strFlashState.u16LastAccessId != 0)
        {
            printf("FlashState:%x:%d,%d\n",
                strFlashState.u16LastAccessId,
                strFlashState.u8Success,
                strFlashState.u8Changed);
