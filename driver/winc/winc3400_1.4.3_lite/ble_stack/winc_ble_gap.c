...
{
    at_ble_status_t status = platform_init(handle);
...
at_ble_status_t at_ble_addr_set(at_ble_addr_t *address)
{
    ...
    if (address->type > AT_BLE_ADDRESS_RANDOM_PRIVATE_NON_RESOLVABLE)
    {
        return AT_BLE_INVALID_PARAM;
    }
...
{
    return gapc_set_tx_pwr_req_handler(conn_handle, (uint8_t) powerdBm);
}

at_ble_status_t at_ble_tx_power_get(at_ble_handle_t conn_handle)
{
    return gapc_get_tx_pwr_req_handler(conn_handle, 0);
}

at_ble_status_t at_ble_max_PA_gain_get(at_ble_handle_t conn_handle)
{
    return gapc_get_tx_pwr_req_handler(conn_handle, 1);
}
...
{
    return gapc_get_info_cmd_handler(conn_handle, GAPC_GET_CON_RSSI);
}
